...
void main() {
  ...
  group('getCourse', () {
    test(
      'should return [Course] when call to remote source is successful',
      () async {
        when(() => remoteDataSrc.getCourse(any())).thenAnswer(
          (_) async => tCourse,
        );
        final result = await repoImpl.getCourse('1');
        expect(result, Right<dynamic, Course>(tCourse));
        verify(() => remoteDataSrc.getCourse('1')).called(1);
        verifyNoMoreInteractions(remoteDataSrc);
      },
    );
    ...
    test(
      ...
      'unsuccessful',
      () async {
        when(() => remoteDataSrc.getCourse(any())).thenThrow(tException);
        final result = await repoImpl.getCourse('1');
        expect(
          result,
