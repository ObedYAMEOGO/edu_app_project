...
import 'package:edu_app_project/src/exams/data/models/question_choice_model.dart';
...
class ExamRemoteDataSrcImpl implements ExamRemoteDataSrc {
      ...
      if (questions != null && questions.isNotEmpty) {
        ...
        for (final question in questions) {
          ...
          var questionToUpload = (question as ExamQuestionModel).copyWith(
            ...
            // We don't really need to pass this, since we already do in the
            // examModel, but we do it anyway for consistency
    ...
    try {
      ...
      if (user == null) {
        throw const ServerException(
          message: 'Cet utilisiteur n\'est pas authentifié',
          statusCode: '401',
        );
      }
      await _firestore
      ...
      final totalPoints = exam.answers
          ...
          .fold<int>(0, (previousValue, element) => previousValue + 1);

      // with this approach, if there were 10 questions, and the user got 5
      // correct, they will get 50% of the points, so, the [points] variable
      // will be 50
  ...
  Future<List<ExamQuestionModel>> getExamQuestions(Exam exam) async {
    try {
      ...
      final result = await _firestore
          ...
          .get();
      return result.docs
          .map((e) => ExamQuestionModel.fromMap(e.data()))
          .toList();
    } on FirebaseException catch (e) {
      throw ServerException(
        message: e.message ?? 'Erreur inconnue',
  ...
  Future<List<UserExamModel>> getUserCourseExams(String courseId) async {
    try {
      ...
      if (user == null) {
        throw const ServerException(
          message: 'Cet utilisateur n\'est pas authentifié',
          statusCode: '401',
        );
      }
      final result = await _firestore
